...
/* Индикаторы состояния */
#define STATUS_LED_GPIO              GPIO_NUM_1    // Status LED (GPIO2)
#define CONNECTION_LED_GPIO          GPIO_NUM_1    // Connection LED (GPIO3)

/* Реле управления нагрузкой */
#define RELAY_1_GPIO                 GPIO_NUM_19    // Relay 1 (GPIO4)
#define RELAY_2_GPIO                 GPIO_NUM_18    // Relay 2 (GPIO5)
...
typedef enum {
    LED_OFF = 0,                     // LED выключен
    LED_ON = 1,                      // LED включен
    LED_BLINK_FAST,                  // Быстрое мигание
    LED_BLINK_SLOW,                  // Медленное мигание
    LED_BLINK_VERY_SLOW              // Очень медленное мигание
} led_state_t;
...
typedef struct {
    ...
    led_state_t status_led_state;    // Состояние LED статуса
    led_state_t connection_led_state; // Состояние LED подключения
